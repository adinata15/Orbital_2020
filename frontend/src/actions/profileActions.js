...
export const getStripeSeller = (state, oauth) => async dispatch => {
  try {
    ...
    await dispatch(
      setAlert('Successfully created a stripe account', 'success')
    );
  } catch (err) {
    dispatch(setAlert('Failed to create a stripe account', 'danger'));
  }
};
...
export const getListings = sellerId => async dispatch => {
  try {
    ...
    dispatch(setAlert('Failed to get listings', 'danger'));
  }
};
